...
eurostat <- eurostat %>%
  ...
  filter(time == "2019", unit == "PC_CHE: Percentual share of total current health expenditure (CHE)", scheme == "HF3: Household out-of-pocket payment") %>% 
  select(-time, -flag) %>% 
...
Det er noe svak korrelasjon som vist. skal putte inn output fra R\^2 og slikt.

Det vi ser er at hvis pps øker, så øker også egenbetalingen. Dette kan være fordi folk har mer penger og dermed bruker mer på helsetjenester. Eller fordi den økte egenbetalingen reduserer bruk av helsetjenester, slik at problemer blir større over tid og folk må bruke mer penger på helsetjenester som øker kostnaden til staten.
